#!/usr/bin/env node
// Build the downloadable mechanical-profile reference WAV.
...
const K = browser.KD;
const profile = K.SYNTH_KEYBOARDS.mechanical;
const { samples } = K.synth.renderStream('demo', 84219, K.CFG.segment.windowMs, {
  keysPerSecond: 4, backgroundNoise: 0.002,
  keyboardType: 'mechanical', keyboardSeed: profile.seed,
});
...
console.log(`Wrote mechanical reference WAV (${(wav.length / 1024).toFixed(1)} KiB).`);
